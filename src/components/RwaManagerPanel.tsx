...
export function RwaManagerPanel() {
  ...
  const [requestIdToTokenId, setRequestIdToTokenId] = useState<{[key: string]: string}>({});
  ...
  useWatchContractEvent({
    ...
    onLogs(logs) {
      logs.forEach((log) => {
        const { args } = log as any; // Type assertion for event args
        if (args?.from === '0x0000000000000000000000000000000000000000' && args?.to === address) {
          // This is a mint to the current user
          ...
          if (tokenId) {
            ...
            // Since we don't have direct access to requestId in the event,
            // we'll use the most recent requestId from state as a fallback
            if (requestId) {
              mapping[requestId] = tokenId;
              localStorage.setItem('requestIdToTokenId', JSON.stringify(mapping));
              setRequestIdToTokenId(mapping);
            }
          }
        }
      });
  ...
  // Load stored mapping on component mount
  useEffect(() => {
    const stored = localStorage.getItem('requestIdToTokenId') || '{}';
    setRequestIdToTokenId(JSON.parse(stored));
  }, []);
...
function AssetInfo({ _requestId }: { _requestId: string }) {
  ...
  // Get NFT Token URI if we have the token ID
  ...
  // Load token ID from localStorage mapping
  useEffect(() => {
    if (_requestId) {
      const stored = localStorage.getItem('requestIdToTokenId') || '{}';
      const mapping = JSON.parse(stored);
      if (mapping[_requestId]) {
        setTokenId(mapping[_requestId]);
      }
    }
  }, [_requestId]);

  // Fetch metadata including IPFS URLs from backend
  useEffect(() => {
    if (_requestId) {
      fetch(`${BACKEND_URL}/assets/metadata/${_requestId}`)
        .then(res => res.json())
        .then(data => {
          if (data.metadata) {
  ...
  return (
    <div style={{ padding: 8, background: '#fafafa', border: '1px solid #eee', borderRadius: 6 }}>
      ...
      {/* NFT Information */}
      {tokenId && (
        <div style={{ marginTop: 8, padding: 8, background: '#f0f8ff', border: '1px solid #cce7ff', borderRadius: 4 }}>
          <strong>NFT Details:</strong>
          <div>Token ID: {tokenId}</div>
          {nftUri && (
            <div>
              URI: <a 
                href={nftUri as string} 
                target="_blank" 
      ...
      {metadata && metadata.ipfsUrls && metadata.ipfsUrls.length > 0 && (
        <div style={{ marginTop: 8 }}>
          <strong>Asset Files (IPFS):</strong>
          <ul style={{ margin: '4px 0', paddingLeft: 20 }}>
            {metadata.ipfsUrls.map((url: string, i: number) => (
      ...
      {metadata && metadata.localFiles && metadata.localFiles.length > 0 && (
        <div style={{ marginTop: 8 }}>
          <strong>Local Files:</strong>
          <ul style={{ margin: '4px 0', paddingLeft: 20 }}>
            {metadata.localFiles.map((file: string, i: number) => (
              <li key={i} style={{ marginBottom: 4 }}>
                <a 
